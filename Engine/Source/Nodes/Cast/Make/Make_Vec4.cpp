...
NODES::CAST::MAKE::Vec4::Vec4() :
	Node(Node_Type::CAST_MAKE_VEC4, "Make")
{
	rect.setWidth(60);
	rect.setHeight(120);
	...
	do_val = DATA_O("", VAR_TYPE::VEC4);
}

Ptr_S<Variable> NODES::CAST::MAKE::Vec4::getData(const Port* port) {
	return make_shared<Variable>(F64_V4(*di_x->GET_DATA(F64), *di_y->GET_DATA(F64), *di_z->GET_DATA(F64), *di_w->GET_DATA(F64)));
}