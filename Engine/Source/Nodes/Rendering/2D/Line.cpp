	...
	Node(Node_Type::RENDER_2D_LINE, "Line")
{
	...
	ei_exec    = EXEC_I("");
	di_vert_a  = DATA_I("A", Variable(F64_V2(-200, -200)));
	di_vert_b  = DATA_I("B", Variable(F64_V2( 200,  200)));
	di_width   = DATA_I("Width", Variable(3.0));
...
void NODES::RENDERING::DIM_2D::Line::render() {
	...
	const auto radius  = to_F32(*di_width ->GET_DATA(F64));
