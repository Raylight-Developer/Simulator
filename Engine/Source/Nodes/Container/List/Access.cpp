...
NODES::CONTAINER::LIST::Access::Access() :
	Node(Node_Type::NONE, "Access")
{
	header_color = QColor(25, 25, 75);
	rect.setWidth(120);
	rect.setHeight(80);

	list  = DATA_I_C("", VAR_TYPE::NONE, VAR_CONTAINER::LIST);
	index = DATA_I("Index", VAR_TYPE::INT);

	value = DATA_O("", VAR_TYPE::NONE);
...
Ptr_S<Variable> NODES::CONTAINER::LIST::Access::getData(const Port* port) {
	const I64 list_index = *index->GET_DATA(I64);
	switch (var_type->currentIndex()) {
		case 1: {
			auto list_data = list->GET_LIST(F64);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 2: {
			auto list_data = list->GET_LIST(I64);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 3: {
			auto list_data = list->GET_LIST(bool);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 4: {
			auto list_data = list->GET_LIST(QString);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 5: {
			auto list_data = list->GET_LIST(F64_V2);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 6: {
			auto list_data = list->GET_LIST(F64_V3);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 7: {
			auto list_data = list->GET_LIST(F64_V4);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 8: {
			auto list_data = list->GET_LIST(Color);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 9: {
			auto list_data = list->GET_LIST(F64_Quat);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 10: {
			auto list_data = list->GET_LIST(F64_M2);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 11: {
			auto list_data = list->GET_LIST(F64_M3);
			return make_shared<Variable>(list_data[list_index]);
		}
		case 12: {
			auto list_data = list->GET_LIST(F64_M4);
			return make_shared<Variable>(list_data[list_index]);
		}
	}
	return nullptr;
